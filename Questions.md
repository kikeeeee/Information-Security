...
<details>
<summary><  Descrivere l'attacco con estensione alle funzioni hash crittograficamente sicure fornendo anche schemi di spiegazione
/summary>
L’attacco con estensione alle funzioni hash crittograficamente sicure riguarda tutti quei casi in cui la funzione hash è implementata seguendo uno schema di compressione iterata. Nel caso in cui la funzione hash sia utilizzata per autenticare l’origine di un messaggio (creando appunto un attestato di autenticità mediante l’impronta di s||m, dove s è un segreto condiviso tra mittente e destinatario ed m il messaggio da autenticare) un intrusore a partire da un attestato di autenticità di questo tipo, e costruito mediante uno schema di compressione iterata, potrebbe autenticare facilmente anche un messaggio così costruito: m||m’, in cui m’ è un’estensione di sua scelta al messaggio. Per farlo gli basta utilizzare l’attestato di autenticità di m come impronta calcolata al passo precedente rispetto al passo in cui viene calcolata l’impronta del primo blocco dell’estensione m’. Per cui concatenando l’attestato di autenticità di m con il primo blocco dell’estensione del messaggio m’ può proseguire, seguendo lo schema di compressione iterata, al calcolo di un attestato di autenticità corretto per m||m’.
Anche nel caso in cui l’algoritmo di compressione iterata preveda l’uso di padding, l’attacco con estensione rimane comunque pericoloso in quei casi in cui non si può stabilire se l’input che si aspetta il destinatario è dotato di significato o meno, come nel caso di input di tipo numerico.
Il modo più sicuro per evitare un attacco di questo tipo è calcolare l’attestato di autenticità concatenando il segreto condiviso s dopo il messaggio m, quindi calcolando l’impronta di H(m||s).
